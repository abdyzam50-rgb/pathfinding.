...
public class PathLimiters {
     ...
     */
    public static boolean isPhysicalSpacePassable(Level world, BlockPos pos, BlockPos start, BlockPos end) {
        if (!PathingEnvironment.isPassable(world, pos, start, end) ||
                !PathingEnvironment.isPassable(world, pos.above(), start, end)) {
            return false;
        }

        // Structural wall limitation checks (allows stairs and slabs)
        if (!world.getBlockState(pos).isAir() && world.getBlockState(pos).isCollisionShapeFullBlock(world, pos)) {
            return false;
        }
     ...
     */
    public static boolean executeBresenhamCheck(Level world, BlockPos p1, BlockPos p2,
