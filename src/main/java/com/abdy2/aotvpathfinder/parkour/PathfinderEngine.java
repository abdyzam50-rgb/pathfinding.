...
public class PathfinderEngine {
    ...
    public static List<PathNode> findNodePath(Level world, BlockPos start, Vec3 startPrecise,
    ...
    public static List<PathNode> findSprintNodePath(Level world, BlockPos start, Vec3 startPrecise,
                                                     BlockPos end, boolean hasWaterBucket) {
     ...
     */
    private static BlockPos resolveValidStart(Level world, BlockPos candidate, Vec3 precise) {
    ...
    private static List<PathNode> runAStar(Level world, BlockPos start, Vec3 startPrecise,
    ...
    private static List<PathNode> runAStar(Level world, BlockPos start, Vec3 startPrecise,
        ...
        Vec3 preciseStart = (startPrecise != null) ? startPrecise
     ...
     */
    private static List<PathNode> getNeighbors(Level world, PathNode current,
        ...
        if (canClimbTo(world, pos, climbUp, pathStart, pathEnd)) {
            Vec3 precise = new Vec3(pos.getX() + 0.5, climbUp.getY(), pos.getZ() + 0.5);
        ...
        if (canClimbDown(world, pos, climbDown, pathStart, pathEnd)) {
            Vec3 precise = new Vec3(pos.getX() + 0.5, climbDown.getY(), pos.getZ() + 0.5);
        ...
        if (hasBuildingBlocks && hasSolidGround(world, pos)) {
            BlockPos pillarTarget = pos.above();
            BlockPos pillarHead   = pos.above(2);
            if (passable(world, pillarTarget, pathStart, pathEnd)
                ...
                Vec3 precise = new Vec3(pos.getX() + 0.5, pillarTarget.getY(), pos.getZ() + 0.5);
    ...
    private static void addHorizontalOptions(Level world, PathNode current, int dx, int dz,
                                             BlockPos pathStart, BlockPos pathEnd,
                                             boolean sprintMode, boolean hasWaterBucket,
                                             boolean hasBuildingBlocks,
                                             List<PathNode> out) {
        ...
        if (canWalkTo(world, from, target, pathStart, pathEnd)) {
            Vec3 precise = new Vec3(target.getX() + 0.5, target.getY(), target.getZ() + 0.5);
            ...
            Vec3 precise = new Vec3(target.getX() + 0.5, target.getY(), target.getZ() + 0.5);
            out.add(new PathNode(target, precise, current, PathNode.Type.BRIDGE, target.below()));
        }
        ...
        if (canJumpTo(world, from, target, jumpLanding, pathStart, pathEnd)) {
            Vec3 precise = new Vec3(target.getX() + 0.5, jumpLanding.getY(), target.getZ() + 0.5);
        ...
        if (!diagonal) {
            ...
            if (landing != null) {
                boolean isSlime = world.getBlockState(landing.below()).getBlock() instanceof SlimeBlock;
            ...
            for (int hFwd = 2; hFwd <= 3; hFwd++) {
                ...
                if (sd != null) {
                    boolean isSlime = world.getBlockState(sd.below()).getBlock() instanceof SlimeBlock;
                    Vec3 precise = new Vec3(sd.getX() + 0.5, sd.getY(), sd.getZ() + 0.5);
        ...
        if (!diagonal) {
            ...
            for (int yDrop = 1; yDrop <= 3; yDrop++) {
                ...
                for (int dist = 1; dist <= maxDist; dist++) {
                    ...
                    if (ok) {
                        BlockPos landing = from.offset(dx * dist, -yDrop, dz * dist);
                        Vec3 precise = new Vec3(landing.getX() + 0.5, landing.getY(), landing.getZ() + 0.5);
                        out.add(new PathNode(landing, precise, current, PathNode.Type.SPRINT_JUMP));
                        break;
                    }
            ...
            if (canNeoJump(world, from, dx, dz, pathStart, pathEnd)) {
                BlockPos landing = from.offset(dx * NEO_JUMP_DIST, 0, dz * NEO_JUMP_DIST);
                Vec3 precise = new Vec3(landing.getX() + 0.5, landing.getY(), landing.getZ() + 0.5);
                out.add(new PathNode(landing, precise, current, PathNode.Type.SPRINT_JUMP));
            }
        }
        ...
        if (!diagonal && PathingEnvironment.isClosedOpenableDoor(world, target)) {
            BlockPos throughDoor = from.offset(dx * 2, 0, dz * 2);
            if (passable(world, throughDoor, pathStart, pathEnd)
                    && passable(world, throughDoor.above(), pathStart, pathEnd)
                    && hasSolidGround(world, throughDoor)) {
                Vec3 precise = new Vec3(target.getX() + 0.5, from.getY(), target.getZ() + 0.5);
        ...
        if (!diagonal && hasWaterBucket) {
            ...
            if (wcLanding != null && (from.getY() - wcLanding.getY() > MAX_FALL_BLOCKS)) {
                Vec3 precise = new Vec3(wcLanding.getX() + 0.5, wcLanding.getY(), wcLanding.getZ() + 0.5);
     ...
     */
    private static boolean canWalkTo(Level world, BlockPos from, BlockPos to,
                                     BlockPos pathStart, BlockPos pathEnd) {
        ...
        if (PathingEnvironment.isHazardous(world, to.below())) return false;
     ...
     * 1-wide gap (target.below()) to let the player walk across at the same Y level.
     */
    private static boolean canBridgeTo(Level world, BlockPos target,
                                        BlockPos pathStart, BlockPos pathEnd) {
        ...
        if (!world.getBlockState(target.below()).isAir()) return false;      // gap must be air (not water/lava)
        if (!passable(world, target.above(), pathStart, pathEnd)) return false; // need headroom at destination
     ...
     */
    private static boolean canJumpTo(Level world, BlockPos from, BlockPos stepBlock,
     ...
     */
    private static boolean canSprintJumpTo(Level world, BlockPos from, int dx, int dz, int dist,
                                           BlockPos pathStart, BlockPos pathEnd) {
        ...
        // into mid.above(1) space — passable() misses this because the block at mid.above(1)
    ...
    public static boolean canSprintJumpToWithPlacedGround(Level world, BlockPos from, int dx, int dz,
        ...
        BlockPos landing = from.offset(dx * dist, 0, dz * dist);
        if (!landing.below().equals(placedGround)) return false;
        if (!passable(world, landing, pathStart, pathEnd)) return false;
        if (!passable(world, landing.above(), pathStart, pathEnd)) return false;
        if (PathingEnvironment.isHazardous(world, landing)) return false;
     ...
     */
    private static boolean canSprintJumpFlat(Level world, BlockPos from, int dx, int dz, int dist,
                                             BlockPos pathStart, BlockPos pathEnd) {
        if (!hasSolidGround(world, from)) return false;
        if (!passable(world, from.above(1), pathStart, pathEnd)) return false;
        if (!passable(world, from.above(2), pathStart, pathEnd)) return false;
     ...
     */
    private static BlockPos findDropLanding(Level world, BlockPos from, BlockPos target,
                                            BlockPos pathStart, BlockPos pathEnd) {
        ...
        for (int drop = 1; drop <= MAX_FALL_BLOCKS; drop++) {
            ...
            if (PathingEnvironment.isHazardous(world, land.below())) return null;
     ...
     */
    private static BlockPos findSprintDropLanding(Level world, BlockPos from, int dx, int dz,
        ...
        for (int i = 1; i <= hForward; i++) {
            BlockPos col = from.offset(dx * i, 0, dz * i);
            if (!passable(world, col, pathStart, pathEnd)) return null;
            if (!passable(world, col.above(), pathStart, pathEnd)) return null;
        }
        // The far column must NOT have solid ground at player level (it is the drop-off edge)
        BlockPos farCol = from.offset(dx * hForward, 0, dz * hForward);
    ...
    private static boolean canClimbTo(Level world, BlockPos from, BlockPos to,
                                      BlockPos pathStart, BlockPos pathEnd) {
     ...
     */
    private static boolean canClimbDown(Level world, BlockPos from, BlockPos to,
                                        BlockPos pathStart, BlockPos pathEnd) {
        if (to.getY() != from.getY() - 1) return false;
        if (to.getX() != from.getX() || to.getZ() != from.getZ()) return false;
     ...
     */
    private static boolean canSprintJumpUphill(Level world, BlockPos from, int dx, int dz,
                                                int dist, BlockPos pathStart, BlockPos pathEnd) {
        // Landing is always exactly +1 Y — reject if a 2-block wall blocks the arc.
        BlockPos landing = from.offset(dx * dist, 1, dz * dist);
        BlockPos step = from.offset(dx * dist, 0, dz * dist);
        if (!passable(world, step, pathStart, pathEnd)
                && !passable(world, step.above(), pathStart, pathEnd)) {
            return false;
        }
        if (!hasSolidGround(world, from)) return false;
     ...
     */
    private static boolean canSprintJumpDownhill(Level world, BlockPos from, int dx, int dz,
                                                  int dist, BlockPos pathStart, BlockPos pathEnd) {
        if (!hasSolidGround(world, from)) return false;
        if (!passable(world, from.above(1), pathStart, pathEnd)) return false;
        if (!passable(world, from.above(2), pathStart, pathEnd)) return false;
        ...
        BlockPos targetSameY = from.offset(dx * dist, 0, dz * dist);
     ...
     */
    private static boolean canSprintJumpDeepDrop(Level world, BlockPos from, int dx, int dz,
                                                  int dist, int yDrop,
                                                  BlockPos pathStart, BlockPos pathEnd) {
        if (!hasSolidGround(world, from)) return false;
        if (!passable(world, from.above(1), pathStart, pathEnd)) return false;
     ...
     */
    private static boolean canSprintJumpThenDrop(Level world, BlockPos from, int dx, int dz,
                                                  int dist, int yDrop,
                                                  BlockPos pathStart, BlockPos pathEnd) {
        if (!hasSolidGround(world, from)) return false;
        if (!passable(world, from.above(1), pathStart, pathEnd)) return false;
     ...
     */
    private static boolean canNeoJump(Level world, BlockPos from, int dx, int dz,
                                       BlockPos pathStart, BlockPos pathEnd) {
        if (!hasSolidGround(world, from)) return false;
        ...
        BlockPos neoBlock = from.offset(dx, 0, dz);
        if (passable(world, neoBlock, pathStart, pathEnd)) return false;  // no wall → no neo
        if (!passable(world, neoBlock.above(1), pathStart, pathEnd)) return false;
        if (!passable(world, neoBlock.above(2), pathStart, pathEnd)) return false;
     ...
     */
    private static void addBounceNeighbors(Level world, PathNode bounceNode,
                                            BlockPos pathStart, BlockPos pathEnd,
                                            List<PathNode> out) {
        ...
        for (int bx = -reach; bx <= reach; bx++) {
            for (int bz = -reach; bz <= reach; bz++) {
                ...
                BlockPos land = slimeSurface.offset(bx, 0, bz);
                if (passable(world, land, pathStart, pathEnd)
                        && passable(world, land.above(), pathStart, pathEnd)
                        && hasSolidGround(world, land)
                        && !PathingEnvironment.isHazardous(world, land)) {
                    Vec3 precise = new Vec3(land.getX() + 0.5, land.getY(), land.getZ() + 0.5);
                ...
                for (int by = 1; by <= maxUpStep; by++) {
                    ...
                    BlockPos landUp = slimeSurface.offset(bx, by, bz);
                    if (passable(world, landUp, pathStart, pathEnd)
                            && passable(world, landUp.above(), pathStart, pathEnd)
                            && hasSolidGround(world, landUp)
                            && !PathingEnvironment.isHazardous(world, landUp)) {
                        Vec3 precise = new Vec3(landUp.getX() + 0.5, landUp.getY(), landUp.getZ() + 0.5);
     ...
     */
    private static BlockPos findDropLandingWaterClutch(Level world, BlockPos from, BlockPos target,
                                                       BlockPos pathStart, BlockPos pathEnd) {
        if (!passable(world, target, pathStart, pathEnd)) return null;
        if (!passable(world, target.above(), pathStart, pathEnd)) return null;
        if (!hasSolidGround(world, from)) return null;
     ...
     */
    private static boolean canCutCorner(Level world, BlockPos from, int dx, int dz,
                                        BlockPos pathStart, BlockPos pathEnd) {
    ...
    /**
     * True if a fence or wall at pos.below() extends upward into pos.
     *
     ...
     */
    private static boolean isHitboxSegmentClear(Level world, BlockPos from, BlockPos to,
                                                BlockPos pathStart, BlockPos pathEnd) {
    ...
    private static List<PathNode> adjustSprintJumpTakeoffPositions(List<PathNode> path) {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            ...
            Vec3 launch = curr.getFeetPos();
            Vec3 land   = next.getFeetPos();
            ...
            result.set(i, new PathNode(curr.pos,
                    new Vec3(optX, curr.pos.getY(), optZ),
                    null, curr.type));
        }
        return result;
    }
    ...
    private static List<PathNode> smoothPrecisePositions(List<PathNode> path) {
        ...
        for (int i = 1; i < path.size() - 1; i++) {
            ...
            if ((curr.type == PathNode.Type.WALK || curr.type == PathNode.Type.EDGE)
                ...
                Vec3 pv = prev.getCenterPos();
                Vec3 nv = next.getCenterPos();
                    ...
                    for (int s = 1; s <= strides; s++) {
                        ...
                        if (!wPos.equals(from.pos) && !wPos.equals(to.pos)) {
                            result.add(new PathNode(wPos,
                                    new Vec3(exactX, strideY, exactZ),
     ...
     */
    private static List<PathNode> simplifyPath(Level world, List<PathNode> path,
                                               BlockPos pathStart, BlockPos pathEnd) {
        if (path.size() <= 2) return path;
    ...
    private static boolean canMoveDirectly(Level world, PathNode from, PathNode to,
                                           BlockPos pathStart, BlockPos pathEnd) {
