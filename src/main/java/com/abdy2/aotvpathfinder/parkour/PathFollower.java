...
public class PathFollower {
    ...
    private static volatile net.minecraft.core.BlockPos            pendingReplanGoalCap = null;
    ...
    private static net.minecraft.core.BlockPos   previewGoal   = null;
    private static boolean                             previewSprint = false;

    public static void storePreview(List<PathNode> path,
                                    net.minecraft.core.BlockPos goal, boolean sprint) {
        previewPath   = path;
        previewGoal   = goal;
    ...
    public static net.minecraft.core.BlockPos getPreviewGoal() { return previewGoal; }
    ...
    private static net.minecraft.core.BlockPos replanGoal         = null;
    ...
    public static void setReplanGoal(net.minecraft.core.BlockPos goal, boolean sprint) {
        replanGoal   = goal;
        replanSprint = sprint;
        replanCooldown    = 0;
        consecutiveStuck  = 0;
    ...
    private static Vec3 lastStuckCheckPos         = null;
    ...
    public static boolean startFollowing(Player player, Level world, BlockPos targetPos) {
        if (player == null || world == null || targetPos == null) return false;
        currentPath = PathfinderEngine.findNodePath(world, player.blockPosition(), targetPos);
    ...
    public static void tick(Player player) {
        ...
        Vec3 targetCenter  = targetNode.getCenterPos();
        ...
        if (targetNode.type == PathNode.Type.INTERACT && targetNode.interactPos != null) {
            Vec3 toBlock = Vec3.atCenterOf(targetNode.interactPos)
        ...
        if (currentNodeIndex + 1 < currentPath.size()
            ...
            Vec3 nextCenter = nextNode.getCenterPos();
        ...
        if (targetNode.type == PathNode.Type.CLIMB) {
            ...
            reached = targetNode.interactPos != null
                    && mc.level != null
                    && !mc.level.getBlockState(targetNode.interactPos).isAir()
        ...
        if (reached) {
            ...
            if (targetNode.type == PathNode.Type.WATER_DROP) {
                ...
                player.setXRot(80.0f); // look steeply down toward the water block
        ...
        if (targetNode.type == PathNode.Type.DROP && !player.onGround()) {
            setKey(mc.options.keyUp, true);
            setKey(mc.options.keyDown,    false);
            setKey(mc.options.keyLeft,    false);
            setKey(mc.options.keyRight,   false);
        ...
        if (targetNode.type == PathNode.Type.SPRINT_JUMP) {
            if (player.onGround()) {
                ...
                if (jumpCooldown <= 0 && hDist > 0.5 && (jumpWillLand || forceJump)) {
                    mc.options.keyJump.setDown(true);
                    jumpCooldown = 6;
                    sprintJumpLaunchYaw = player.getYRot(); // lock at exact moment of launch
                ...
                player.setYRot(sprintJumpLaunchYaw);
        ...
        if (targetNode.type == PathNode.Type.BOOST_PLACE) {
            if (player instanceof LocalPlayer cpe) selectBlockSlot(cpe);
            if (player.onGround()) {
                faceAndWalk(player, mc, dx, dz, true);
                applyLandingPitch(player, hDist, dy);
                if (jumpCooldown <= 0 && hDist > 0.5) {
                    mc.options.keyJump.setDown(true);
                    jumpCooldown = 6;
                    sprintJumpLaunchYaw = player.getYRot();
                }
            } else {
                player.setYRot(sprintJumpLaunchYaw);
                setKey(mc.options.keyUp, true);
        ...
        if (targetNode.type == PathNode.Type.PILLAR && targetNode.interactPos != null) {
            ...
            if (player.onGround()) {
                ...
                BlockPos floorBlock = placePos.below();
                if (mc.level != null && !mc.level.getBlockState(floorBlock).isAir()
                        && player instanceof LocalPlayer cpe
                        && mc.gameMode != null) {
                    selectBlockSlot(cpe);
                    player.setXRot(85f);
                    Vec3 hitVec = new Vec3(floorBlock.getX() + 0.5, floorBlock.getY() + 1.0,
                                             floorBlock.getZ() + 0.5);
                    mc.gameMode.useItemOn(cpe, InteractionHand.MAIN_HAND,
        ...
        // interactPos = the air gap block (target.below()); placed by clicking the side face
        ...
        if (targetNode.type == PathNode.Type.BRIDGE && targetNode.interactPos != null) {
            ...
            if (mc.level != null && !mc.level.getBlockState(gapPos).isAir()) {
            ...
            BlockPos playerFloor = player.blockPosition().below();
            ...
            if (player instanceof LocalPlayer cpe
                    && mc.gameMode != null
                    && mc.level != null
                    && !mc.level.getBlockState(playerFloor).isAir()) {
                selectBlockSlot(cpe);
                Vec3 hitVec = Vec3.atCenterOf(playerFloor).relative(placeDir, 0.5);
                mc.gameMode.useItemOn(cpe, InteractionHand.MAIN_HAND,
                        new BlockHitResult(hitVec, placeDir, playerFloor, false));
            }

            // Walk toward the gap while sneaking so player doesn't fall in
            setKey(mc.options.keyShift, true);
            faceAndWalk(player, mc, dx, dz, false);
            return;
        }
        ...
        if (targetNode.type == PathNode.Type.CLIMB) {
            ...
            if (mc.level != null) {
                var b1 = mc.level.getBlockState(player.blockPosition()).getBlock();
                var b2 = mc.level.getBlockState(targetNode.pos).getBlock();
            ...
            if (!goingUp) {
                ...
                setKey(mc.options.keyShift,  false);
                setKey(mc.options.keyDown,   false);
                mc.options.keyJump.setDown(true); // jump is the universal "go up" key

                switch (climbType) {
                    case LADDER -> {
                        // Snap yaw toward the ladder face; W is NOT needed — jump alone climbs
                        setKey(mc.options.keyUp, false);
                        snapToLadderFace(player, player.blockPosition(), targetNode.pos, mc);
                    }
        ...
        if (targetNode.type == PathNode.Type.WATER_DROP) {
            ...
            player.setXRot(player.getXRot() + (desiredPitch - player.getXRot()) * 0.5f);
            if (hDist > 0.2) {
                float desiredYaw = (float) Math.toDegrees(Math.atan2(dz, dx)) - 90f;
                player.setYRot(player.getYRot() + normalizeAngle(desiredYaw - player.getYRot()) * 0.4f);
            }

            // Walk off the edge while still on ground; stay still in air
            if (player.onGround() && hDist > 0.2) {
                setKey(mc.options.keyUp, true);
                setKey(mc.options.keyDown,    false);
                setKey(mc.options.keyLeft,    false);
                setKey(mc.options.keyRight,   false);
            ...
            // Without the ServerboundSetCarriedItemPacket the server uses the wrong item.
            if (player instanceof net.minecraft.client.player.LocalPlayer cpe) {
                int bucketSlot = findWaterBucketSlot(cpe);
                if (bucketSlot >= 0 && cpe.getInventory().getSelectedSlot() != bucketSlot) {
                    cpe.getInventory().setSelectedSlot(bucketSlot);
                    cpe.connection.send(new ServerboundSetCarriedItemPacket(bucketSlot));
                }
            }
            ...
            if (!waterClutchPlaced && !player.onGround() && eyeDist <= 4.4) {
                double currAboveGround = player.getY() - groundSurface;
                double nextAboveGround = (player.getY() + player.getDeltaMovement().y) - groundSurface;
            ...
            if (player.onGround() && Math.abs(dy) < 1.5) waterClutchPlaced = false;
            return;
        }
        ...
        if (targetNode.type == PathNode.Type.JUMP
                ...
                && player.onGround() && jumpCooldown <= 0) {
            mc.options.keyJump.setDown(true);
            jumpCooldown = 8;
        } else if (jumpCooldown <= 0) {
            mc.options.keyJump.setDown(false);
        ...
        if (player.onGround() && mc.level != null
                && !nextIsSprint
                && !nextIsDrop
                && (targetNode.type == PathNode.Type.WALK
                 || targetNode.type == PathNode.Type.EDGE)) {
            ...
            if (hSpeed > 0.05) {
                ...
                double cliffDist = cliffDistanceAhead(mc.level, player, scanDx, scanDz);
                ...
                if (cliffDist <= stopBudget) {
                    ...
                    player.setYRot(player.getYRot() +
                            normalizeAngle(brakeYaw - player.getYRot()) * 0.35f);
                    setKey(mc.options.keyUp, false);
                    setKey(mc.options.keyDown,    true);
                    setKey(mc.options.keyLeft,    false);
                    setKey(mc.options.keyRight,   false);
                    setKey(mc.options.keySprint,  false);
                    mc.options.keyJump.setDown(false);
        ...
        if (player.onGround()
            ...
            if (hSpeed > 0.15) {
                ...
                float brakeYaw = (float) Math.toDegrees(Math.atan2(dz, dx)) - 90f;
                float brakeDiff = normalizeAngle(brakeYaw - player.getYRot());
                player.setYRot(player.getYRot() + brakeDiff * 0.35f);
                setKey(mc.options.keyUp, false);
                setKey(mc.options.keyDown,    true);
                setKey(mc.options.keyLeft,    false);
                setKey(mc.options.keyRight,   false);
        ...
        if (cumulativeDist != null
                && (targetNode.type == PathNode.Type.WALK
                 || targetNode.type == PathNode.Type.EDGE
                 || targetNode.type == PathNode.Type.SPRINT_JUMP)) {
            double hSpeed = Math.sqrt(player.getDeltaMovement().x * player.getDeltaMovement().x
                                    + player.getDeltaMovement().z * player.getDeltaMovement().z);
            double tPlayer = projectOnPath(player.getX(), player.getZ());
            ...
            Vec3 goal = computeFunnelGoal(player, tPlayer, L);
    ...
    private static double cornerSharpnessAhead(double tPlayer) {
        ...
        for (int i = 0; i < currentPath.size() - 2; i++) {
            ...
            Vec3 a = currentPath.get(i).getFeetPos();
            Vec3 b = currentPath.get(i + 1).getFeetPos();
            Vec3 c = currentPath.get(i + 2).getFeetPos();
     ...
     */
    private static Vec3 computeFunnelGoal(Player player, double tPlayer, double L) {
        if (currentPath == null || cumulativeDist == null) return null;

        Vec3 pos = new Vec3(player.getX(), player.getY(), player.getZ());
        double vx = player.getDeltaMovement().x, vz = player.getDeltaMovement().z;
        ...
        if (hSpeed > 0.15) {
            ...
            Vec3 center = goalPoint(tPlayer + L);
            Vec3 velGoal = pos.add(vx / hSpeed * L, 0, vz / hSpeed * L);
            naturalGoal = center != null
                ? new Vec3(center.x + (velGoal.x - center.x) * w,
                            center.y,
        ...
        for (int i = currentNodeIndex; i < currentPath.size() - 1; i++) {
            ...
            if (Math.abs(lateralOffset) > 0.3) {
                ...
                return new Vec3(B.x + px * clamped, B.y, B.z + pz * clamped);
            }
        }
     ...
     */
    private static boolean tryEscapeHole(Player player, Minecraft mc) {
        if (mc.level == null || !player.onGround()) return false;
        ...
        for (Direction d : new Direction[]{
            ...
            var sfState = mc.level.getBlockState(feet.relative(d));
            var shState = mc.level.getBlockState(feet.above().relative(d));
        ...
        boolean aboveOpen = mc.level.getBlockState(feet.above(2)).isAir();
        if (aboveOpen) {
            mc.options.keyJump.setDown(true);
            if (openDir != null) {
                faceAndWalk(player, mc, openDir.getStepX(), openDir.getStepZ(), false);
            } else {
                setKey(mc.options.keyUp, true);
                setKey(mc.options.keyDown,    false);
                setKey(mc.options.keyLeft,    false);
     ...
     */
    private static void triggerReplan(Player player, Minecraft mc) {
        if (replanGoal == null || mc.level == null) { stopFollowing(); return; }
        if (pendingReplanFuture != null && !pendingReplanFuture.isDone()) return; // already computing

        net.minecraft.core.BlockPos goal = replanGoal;
        boolean sprint  = replanSprint;
        Vec3   precise = new Vec3(player.getX(), player.getY(), player.getZ());
        net.minecraft.core.BlockPos startPos = player.blockPosition();
        net.minecraft.world.level.Level world = mc.level; // read-only use on background thread
        ...
        pendingReplanFuture = PATHFIND_EXECUTOR.submit(() -> sprint
            ? PathfinderEngine.findSprintNodePath(
                    world, startPos, precise, goal, false)
            : PathfinderEngine.findNodePath(
                    world, startPos, precise, goal, false));
    }
    ...
    private static void buildArcLengths(List<PathNode> path) {
        ...
        for (int i = 1; i < path.size(); i++) {
            Vec3 a = path.get(i - 1).getFeetPos();
            Vec3 b = path.get(i).getFeetPos();
     ...
     */
    private static Vec3 goalPoint(double t) {
        if (currentPath == null || cumulativeDist == null) return null;
        ...
        for (int i = 0; i < currentPath.size() - 1; i++) {
            if (t <= cumulativeDist[i + 1]) {
                ...
                return new Vec3(a.x + (b.x - a.x) * frac,
                                 a.y + (b.y - a.y) * frac,
                                 a.z + (b.z - a.z) * frac);
            }
        }
     ...
     */
    private static void snapToLadderFace(Player player, BlockPos playerBlock,
                                         BlockPos targetBlock, Minecraft mc) {
        if (mc.level == null) return;
        var state = mc.level.getBlockState(playerBlock);
        if (!(state.getBlock() instanceof LadderBlock)) {
            state = mc.level.getBlockState(targetBlock);
        }
        ...
        float diff = normalizeAngle(targetYaw - player.getYRot());
        player.setYRot(player.getYRot() + diff * 0.4f);
    }
     ...
     */
    private static void faceAndWalk(Player player, Minecraft mc,
        ...
        float desiredYaw = (float) Math.toDegrees(Math.atan2(dz, dx)) - 90f;
        float diff = normalizeAngle(desiredYaw - player.getYRot());
        ...
        player.setYRot(player.getYRot() + diff * lerpRate);
        ...
        setKey(mc.options.keyLeft,    false);
        setKey(mc.options.keyRight,   false);
        setKey(mc.options.keySprint,  sprint);
    }
     ...
     */
    private static boolean hasDrop(Player player, Minecraft mc,
                                   double dx, double dz) {
        ...
        for (int side = -1; side <= 1; side += 2) {
            ...
            boolean airBeside = mc.level.getBlockState(new BlockPos(bx, fy,     bz)).isAir();
            boolean airBelow  = mc.level.getBlockState(new BlockPos(bx, fy - 1, bz)).isAir();
    ...
    private static void autoInteractDoors(Player player, Minecraft mc,
                                          double dirX, double dirZ) {
        if (mc.level == null) return;
        BlockPos origin = player.blockPosition();

        for (int i = 1; i <= 2; i++) {
            BlockPos ahead = origin.offset(
                    (int) Math.round(dirX * i), 0, (int) Math.round(dirZ * i));
            checkAndOpenDoor(player, mc, ahead);
            checkAndOpenDoor(player, mc, ahead.above());
        }
    }

    private static void checkAndOpenDoor(Player player, Minecraft mc, BlockPos pos) {
        if (mc.level == null) return;
    ...
    private static void interactWithBlock(Player player, BlockPos blockPos) {
        Minecraft mc = Minecraft.getInstance();
        if (!(player instanceof LocalPlayer cpe) || mc.gameMode == null) return;
    ...
    private static void selectBlockSlot(LocalPlayer player) {
        var inv = player.getInventory();
        for (int i = 0; i < 9; i++) {
            ItemStack s = inv.getItem(i);
            if (s.isEmpty() || !(s.getItem() instanceof BlockItem bi)) continue;
            var st = bi.getBlock().defaultBlockState();
    ...
    private static void faceBlock(Player player, BlockPos target) {
        double dx = (target.getX() + 0.5) - player.getX();
        double dz = (target.getZ() + 0.5) - player.getZ();
        player.setYRot(normalizeAngle((float) Math.toDegrees(Math.atan2(dz, dx)) - 90f));
    }
     ...
     */
    private static void applyLandingPitch(Player player, double hDist, double dy) {
        ...
        player.setXRot(player.getXRot() + (target - player.getXRot()) * 0.5f);
    }
     ...
     */
    private static void applyVelocityPitch(Player player) {
        ...
        if (hSpeed > 0.05) {
            ...
            player.setXRot(player.getXRot() + (targetPitch - player.getXRot()) * 0.4f);
        }
    }
     ...
     */
    private static void setPitchToBlock(Player player, BlockPos target) {
        double dx   = (target.getX() + 0.5) - player.getX();
        double dy   = (target.getY() + 1.0) - (player.getY() + 1.62); // block center vs eye
        double dz   = (target.getZ() + 0.5) - player.getZ();
        double hd   = Math.sqrt(dx * dx + dz * dz);
        float desired = (float) -Math.toDegrees(Math.atan2(dy, hd));
        float newPitch = player.getXRot() + (desired - player.getXRot()) * 0.4f;
    ...
    private static void setKey(KeyMapping key, boolean pressed) {
        key.setDown(pressed);
    }
     ...
     */
    private static double cliffDistanceAhead(Level world, Player player,
                                             double nx, double nz) {
     ...
     */
    private static int findWaterBucketSlot(net.minecraft.client.player.LocalPlayer player) {
        var inv = player.getInventory();
        for (int i = 0; i < 9; i++) {
     ...
     */
    private static double estimateSprintJumpDistance(Player player, double dx, double dz,
                                                      double dy) {
        double hdist = Math.sqrt(dx * dx + dz * dz);
