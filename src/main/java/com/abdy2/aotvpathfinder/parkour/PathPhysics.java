...
public class PathPhysics {
    ...
    public static class MicroBlockBB {
         ...
         */
        public AABB toWorldBox(BlockPos blockPos) {
            return new AABB(
     ...
     */
    private static ParkourNode handleHeadHitter(Level world, ParkourNode previous,
     ...
     */
    public static boolean checkHorizontalCollision(Level world, ParkourNode from, ParkourNode to) {
     ...
     */
    public static boolean checkHeadHitter(Level world, ParkourNode node) {
     ...
     */
    private static boolean isBlockCollidableHorizontal(Level world, BlockPos pos,
     ...
     */
    public static BlockPos getGroundBlockBelow(Level world, ParkourNode node) {
        ...
        for (int offset = 0; offset >= -1; offset--) {
            ...
            double blockTopY = candidate.getY() + shape.bounds().maxY;
    ...
    public static boolean checkGroundBelow(Level world, ParkourNode node) {
     ...
     */
    public static double getBlockTopSurface(Level world, BlockPos pos) {
        var state = world.getBlockState(pos);
        VoxelShape shape = state.getCollisionShape(world, pos);
     ...
     */
    public static boolean checkMicroBlockCollision(Level world, BlockPos pos,
                                                    MicroBlockBB microBB,
                                                    double playerX, double playerY, double playerZ) {
        AABB worldBox = microBB.toWorldBox(pos);
    ...
    public static float detectNeoJumpOpportunity(ParkourNode current, BlockPos fencePos,
                                                  Level world, MovementInput input) {
     ...
     */
    public static Vec3 predictLandingPosition(ParkourNode current, MovementInput input, Level world) {
    ...
    public static float findClearYaw(ParkourNode current, MovementInput input,
                                      BlockPos obstaclePos, Level world, float[] yawVariants) {
    ...
    public static class TrajectoryResult {
        ...
        public Vec3 getFinalPosition() {
            if (trajectory.isEmpty()) return Vec3.ZERO;
         ...
         */
        public Vec3 getLandingOrFinalPosition() {
     ...
     */
    private static double getStepHeightNeeded(Level world, ParkourNode from, ParkourNode to) {
        double minX = Math.min(from.x, to.x) - PLAYER_HALF_WIDTH;
        double maxX = Math.max(from.x, to.x) + PLAYER_HALF_WIDTH;
        double minZ = Math.min(from.z, to.z) - PLAYER_HALF_WIDTH;
        double maxZ = Math.max(from.z, to.z) + PLAYER_HALF_WIDTH;
     ...
     */
    private static boolean isMicroTopBlock(Level world, BlockPos pos) {
     ...
     */
    public static List<BlockPos> getTrajectoryIfValid(Level world, BlockPos p1, BlockPos p2,
        ...
        if (horizontalDist < 0.01) {
            ...
            for (int y = p1.getY() - 1; y >= p2.getY(); y--) {
                ...
                if (!isPassable(world, check.above())) return null;
        ...
        while (safetyTicks < 200) {
            ...
            if (!isPassable(world, checkPos.above())) {
                return null;
            }
    ...
    private static boolean isPassable(Level world, BlockPos pos) {
        var state = world.getBlockState(pos);
        return state.isAir() || state.getBlock() instanceof BaseEntityBlock;
    }
}